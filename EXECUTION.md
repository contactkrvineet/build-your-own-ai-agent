...
| **build**  | Builds the Docker image (multi-stage, `runtime` target) and pushes to `ghcr.io`                 |
| **deploy** | Triggers Render.com deploy hooks via `curl` to redeploy the API service (and optionally the UI) |
...
| Secret                   | Description                                              |
| ------------------------ | -------------------------------------------------------- |
| `RENDER_DEPLOY_HOOK_UI`  | Render deploy hook URL for the UI service                |
| `SECRET_KEY`             | Application secret (auto-generated by Render if not set) |
| `OPENAI_API_KEY`         | OpenAI key (if using OpenAI provider)                    |
| `ANTHROPIC_API_KEY`      | Anthropic key (if using Claude)                          |
| `OPENWEATHERMAP_API_KEY` | Weather tool API key                                     |
